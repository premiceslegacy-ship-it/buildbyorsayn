...
These product marks are stored locally so BUILD assets do not depend on a remote CDN at render time.
...
- `anthropic.svg`: Anthropic company mark retained only where the company identity is explicitly named.
- `stripe.svg`, `vercel.svg`, `supabase.svg`: product marks used in delivery and infrastructure scenes.
