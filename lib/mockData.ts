    ...
    sections: [
      ...
      },
      {
        ...
        content:
          ...
          "Le Bloc 4 est entièrement dédié aux skills : la méthode pour les construire (décomposer un métier en sous-compétences, capturer l'expertise, formaliser, chaîner), comment ils s'articulent en système, et comment en faire ton second cerveau. Ici, retiens juste l'essentiel pour comprendre la stack : un Skill se charge à côté des MCP dans ton IDE, et les deux jouent des rôles différents et complémentaires. Le MCP donne à l'IA l'accès à tes données réelles - ton schéma BDD, ton historique Git, tes designs Figma. Le Skill lui donne le jugement pour bien s'en servir - les règles, les critères de décision, les interdits d'un praticien réel. L'un sans l'autre est incomplet : des données réelles jugées avec un jugement générique, ou un bon jugement appliqué à des données inventées.",
      },
    ],
    videos: [
      {
    ...
    sections: [
      ...
      },
      {
        ...
        content:
          ...
          "Couche 2 : les 8 documents fondateurs. L'ordre est strict : ORSAYN-PROJECT.md, BRIEF.md, BRAND-SYSTEM.md, DESIGN-SYSTEM.md, DATA-MODEL.md, SECURITY-MODEL.md, PRD.md, USER-FLOWS.md. Chaque document nourrit le suivant - le BRIEF applique une vraie méthode de positionnement (les alternatives réelles, pas juste les concurrents logiciels) et les 4 forces du changement JTBD ; le PRD pose des critères d'acceptation testables et un plan de tracking, pas des voeux pieux ; USER-FLOWS couvre aussi les flows qu'on oublie toujours - invitation expirée, session concurrente, dernier admin qui part. Une fois le repo initialisé, un CLAUDE.md court est généré depuis ces 8 documents : c'est lui que Claude Code, Cursor ou Codex lisent nativement, pas un neuvième document parallèle qui finirait par diverger.\n\n" +
          ...
          "Couche 4 : les décisions structurantes. Avant de coder, ORACLE force les décisions qui coûtent cher si elles sont prises trop tard : architecture, auth, paiements, intégrations, sécurité, hébergement, automatisations, données modifiables, SEO/GEO. Le pricing, le paywall et l'onboarding s'appuient sur des benchmarks chiffrés (taux d'activation, conversion trial-to-paid, structure de plans qui convertit le mieux) plutôt que sur l'instinct. Vingt minutes ici peuvent éviter deux semaines de correction après.\n\n" +
      ...
      },
      {
        ...
        content:
          ...
          "ux-ui-design est le sous-skill visuel. Dans le dossier complet, tu trouves SKILL.md, une taxonomie de tous les grands styles de design (avec un protocole pour identifier précisément le style d'une référence que tu fournis - couleurs, typo, matière), un protocole anti AI-slop (comment éviter le rendu générique reconnaissable entre mille), une spec Liquid Glass, un catalogue de design systems prêts à l'emploi, des règles copywriting système 1, des quality gates et un agent YAML. Le dossier de références visuelles est volontairement vide : c'est toi qui l'alimentes avec tes propres captures et inspirations - le skill n'a pas de goût imposé, il a une méthode. Il intervient sur BRAND-SYSTEM.md, DESIGN-SYSTEM.md, prompts visuels, maquettes, audits UX/UI, iconographie et copy d'interface, et les mêmes tokens déclinent aussi sur tes supports de communication.\n\n" +
