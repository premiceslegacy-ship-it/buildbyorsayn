`---`  
`name: expert-backend`  
`description: "Expert backend senior pour auditer, concevoir et durcir les backends des projets Orsayn et projets applicatifs : architecture /lib, Supabase/Postgres/RLS, RBAC, sécurité, migrations, Cloudflare/OpenNext, per-client, SaaS, AaaS intégré, cockpit, conformité, webhooks, cron, IA, performance, tests et déploiement client-ready. Utilise ce skill dès que l'utilisateur parle de backend, API, base de données, RLS, auth, sécurité, performance, scalable, dette technique, déploiement, audit, Supabase, Cloudflare Workers, migrations, facturation légale, webhooks, cron, rate limit ou architecture serveur."`  
`---`  
...
`**Mass Assignment — whitelist des colonnes autorisées en écriture :**`  
   
``Ne jamais passer directement l'objet `input` parsé en mutation BDD. Un attaquant peut injecter des champs non prévus (`role`, `plan`, `org_id`, `is_admin`) si la validation ne whitelist pas explicitement les colonnes autorisées.``  
   
`typescript`   
`// ❌ Vulnérable — l'input peut contenir n'importe quelle colonne`   
`const parsed = UpdateUserSchema.parse(input)`   
`await db.from('users').update(parsed).eq('id', userId)`  
   
`// ✅ Sûr — seules les colonnes autorisées passent`   
`const parsed = UpdateUserSchema.parse(input)`   
`await db.from('users').update({`   
 `display_name: parsed.displayName,`   
 `avatar_url:   parsed.avatarUrl,`   
 `// 'role', 'org_id', 'plan' absents — jamais modifiables via cette action`   
`}).eq('id', userId)`  
   
``Règle : chaque mutation construit explicitement l'objet à écrire — jamais de spread `...parsed` sur un update BDD. Le schéma Zod valide le format ; la whitelist garantit le périmètre d'écriture.``  
...
``Tout endpoint qui consomme une URL fournie par l'utilisateur (webhook callback, import depuis une URL, avatar distant, aperçu de lien) est un vecteur SSRF potentiel. Un attaquant peut pointer vers `http://169.254.169.254` (metadata AWS/GCP) ou des services internes.``  
   
`Règles obligatoires si l'app consomme des URLs externes :`  
   
`typescript`   
`// /lib/utils/safe-fetch.ts`   
`import { URL } from 'url'`  
   
`const BLOCKED\_HOSTS \= \[`   
 `/^localhost$/i,`   
 `/^127\\./, /^10\\./, /^172\\.(1\[6-9\]|2\\d|3\[01])\\./,`   
 `/^192\\.168\\./,`   
 `/^169\\.254\\./,   // metadata AWS/GCP/Azure`   
 `/^::1$/,          // IPv6 loopback`   
`\]`  
...
`export function assertSafeUrl(raw: string): URL {`   
 ...
 `if (\!(\['https:', 'http:'\].includes(url.protocol))) throw new Error('Protocole non autorisé')`   
 `if (BLOCKED\_HOSTS.some(re \=\> re.test(url.hostname))) throw new Error('Hôte interne interdit')`   
 `return url`   
`}`  
   
`// Usage dans une Server Action :`   
`// const safeUrl \= assertSafeUrl(input.webhookUrl)`   
`// const res \= await fetch(safeUrl.toString(), { signal: AbortSignal.timeout(5000) })`  
   
`Règles complémentaires :`   
`\* Whitelist de domaines autorisés si le cas d'usage le permet (ex. only cdn.example.com)`   
`\* Timeout court sur tous les appels sortants (AbortSignal.timeout 5s max)`   
`\* Ne jamais logger l'URL complète si elle peut contenir des tokens en query param`   
`\* DNS rebinding : résoudre l'IP en amont et vérifier si le cas est critique`   
`---`  
...
`\[ \] Rate limiting spécifique routes auth (login/reset/magic-link) : max 5/min/IP + backoff exponentiel`   
``\[ \] Mass Assignment : chaque mutation BDD construit un objet explicite — jamais de spread \`...parsed\` sur un update``   
`\[ \] SSRF : toute URL fournie par l'utilisateur validée via assertSafeUrl() avant fetch`   
...
`Lis /docs/orsayn-ecosystem.md pour identifier le mode de déploiement Orsayn.`   
`Pose les 8 questions de calibrage.`   
