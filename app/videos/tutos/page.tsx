...
import { chapterTitle, DoctrineMarkdown, type DoctrineFile } from "../../doctrine/markdown";
import { COFFRE_LABEL, COFFRE_PRICE } from "@/lib/pricing";
import { CHAPTER_META, GROUP_LABELS } from "./chapters";
import { HermesGrid } from "./HermesGrid";
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#1c1c1f] via-[#0e0e0f] to-[#0e0e0f] text-[#f0ede8] font-sans selection:bg-[#e8d5b0]/30 selection:text-[#e8d5b0] relative overflow-x-clip">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-6 sm:py-8 relative z-10">
        <header className="mb-10 flex flex-col sm:flex-row sm:items-start gap-6 sm:gap-8">
                  ...
                  items={group.files.map((file) => ({
                    ...
                    body: (
                      <>
                        ...
                        {CHAPTER_META[file.path]?.diagram}
                      </>
                    ),
                  }))}
                />
